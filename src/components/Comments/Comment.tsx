            ...
            subheader={
              <>
                <Grid container display="flex">
                  <Grid item>
                    <Link
                      href={`/profile/${props.forumComment.author}`}
                      underline="none"
                      color="inherit"
                    >
                      <Typography variant="subtitle2" align={"left"}>
                        {props.forumComment.author &&
                          `By ${props.forumComment.author}`}
                      </Typography>
                    </Link>
                  </Grid>
                  <Grid item sx={{ flex: 1 }}>
                    <Typography variant="subtitle2" align={"right"}>
                      {props.forumComment.updated_at &&
                        `Updated at ${moment(
                          props.forumComment.updated_at
                        ).format("D/M/YYYY")}`}
                    </Typography>
          ...
          <CardActions sx={{ display: "flex", flexDirection: "row" }}>
              ...
              userProfileContextData.userProfile.id === props.forumComment.profile_id && (
                <ButtonGroup>
                  <DefaultIconButton
                    ...
                    onClick={() =>
                      navigate(`/comments/${props.forumComment.id}/edit`)
                    }
            ...
            {props.enableButtons && (
              <Box sx={{ textAlign: "right", flex: 1 }}>
                <DefaultButton
                  onClick={() =>
                    navigate(`/posts/${props.forumComment.post_id}`)
                  }
                  text={"View comment in post"}
                ></DefaultButton>
              </Box>
            )}
          </CardActions>
